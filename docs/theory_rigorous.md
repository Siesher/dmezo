...
## §6.5 Theorem 4 — DP-MeZO-N: privacy/utility trade-off

**Цель:** расширить T3 включением Gaussian DP-noise на ρ. Дать (i) convergence bound с явным DP-term + (ii) privacy guarantee per Gaussian mechanism + (iii) honest composition analysis для $T$ rounds.

### Setup

Расширенный D-MeZO-N step:
$$\tilde\rho_t := \mathrm{clip}(\hat\rho_t, \pm C) + \xi_t, \qquad \xi_t \sim \mathcal{N}(0, \sigma^2) \text{ i.i.d.}$$
$$v_{t+1} = \beta_t v_t + \tilde\rho_t z_t, \qquad \theta_{t+1} = \theta_t - \eta v_{t+1}.$$

То есть к scalar projected gradient после clip'а добавляется Gaussian noise. Параметр $\sigma$ контролирует privacy/utility trade-off.

### Lemma 8 — variance под DP-noise

**Постановка.** Для $\tilde g_t = \tilde\rho_t z_t$ с $z_t \sim \mathcal{N}(0, I)$ и $\xi_t \sim \mathcal{N}(0, \sigma^2)$ независимо:

$$\mathbb{E}\bigl[\|\tilde g_t\|^2\bigr] = \mathbb{E}[\hat\rho^2_{\text{clip}}] \cdot \mathbb{E}\|z\|^2 + \sigma^2 \cdot \mathbb{E}\|z\|^2 \le C^2 d + \sigma^2 d = (C^2 + \sigma^2) d.$$

**Доказательство.**

$\tilde\rho_t = \hat\rho_t^{\text{clip}} + \xi_t$, поэтому
$$\tilde\rho_t^2 = (\hat\rho_t^{\text{clip}})^2 + 2 \hat\rho_t^{\text{clip}} \xi_t + \xi_t^2.$$

Беря $\mathbb{E}_\xi$ (с независимым $\xi$): $\mathbb{E}[\xi_t] = 0$ убирает cross-term, $\mathbb{E}[\xi_t^2] = \sigma^2$:
$$\mathbb{E}_\xi[\tilde\rho_t^2] = (\hat\rho_t^{\text{clip}})^2 + \sigma^2 \le C^2 + \sigma^2.$$

Беря $\mathbb{E}_z$ и используя $\mathbb{E}\|z\|^2 = d$:
$$\mathbb{E}\bigl[\|\tilde g_t\|^2\bigr] = \mathbb{E}[\tilde\rho_t^2 \|z\|^2] \le (C^2 + \sigma^2) d. \quad \square$$

**Принципиальное замечание (Malladi r(H)-трюк "ломается" для DP-noise).**

В Lemma 1(b′) уловка $\mathbb{E}[\hat g^\top M \hat g] \le \|M\|_{op}\|\nabla L\|^2 (r(M)+2)$ работает потому что $\hat\rho \propto \langle\nabla L, z\rangle$ имеет **структуру**, выровненную с градиентом. DP-noise $\xi \cdot z$ — **isotropic в θ-пространстве** (нет alignment с градиентом):

$$\mathbb{E}[(\xi z)^\top M (\xi z)] = \sigma^2 \mathbb{E}[z^\top M z] = \sigma^2 \mathrm{tr}(M).$$

То есть **DP-noise contributes $\sigma^2 \mathrm{tr}(M) = \sigma^2 \cdot r(M) \|M\|_{op}$**, где **здесь $r(M)$ — это полный rank**, не effective rank гессиана (потому что $M=I$ для DP-term).

**Combined bound для descent inequality** ($M = H_*$):
$$\mathbb{E}[\tilde g^\top H_* \tilde g] \le C^2 r(H) \ell + \sigma^2 d \ell$$

(Здесь $d$ = полное число trainable params, **не** $r(H) \ll d$.)

Это **критическое** наблюдение: DP-noise масштабируется с **полной размерностью** $d$, а MeZO-noise — с **effective rank** $r(H) \ll d$. Поэтому даже малый $\sigma$ может численно превзойти $C^2 r(H)$:

$$\sigma^2 d \gtrless C^2 r(H) \quad \Leftrightarrow \quad \sigma \gtrless C\sqrt{r(H)/d}.$$

Для Qwen3.5-0.8B с $d \sim 0.85 \cdot 10^9$, $r(H) \sim 10^2$, $C = 50$:
$$\sigma_{\text{crossover}} \sim 50 \sqrt{10^2/10^9} = 50 \cdot 10^{-3.5} \approx 0.016.$$

То есть **любой $\sigma > 0.02$ теоретически уже доминирует** клип-variance. Эмпирически (см. §6.7) ситуация мягче — likely потому что effective d (через trainable mask) меньше, или потому что step-size мала и degradation накапливается медленно.
...
$$\Delta_{\text{floor}}^{\text{DP}} - \Delta_{\text{floor}}^{\text{no-DP}} = \frac{2 \sigma^2 d \ell}{3\mu}.$$

Это **дополнительная** потеря utility пропорциональная $\sigma^2 \cdot d$ — линейно-quadratic в σ, линейно в полной размерности модели.

### Theorem 4b — Privacy guarantee per single round

**Утверждение (Gaussian mechanism, Dwork-Roth 2014).** Один MeZO-step с clip $C$ и Gaussian noise $\sigma$ обеспечивает $(\varepsilon_1, \delta)$-DP с
$$\varepsilon_1 = \frac{C \sqrt{2 \ln(1.25/\delta)}}{\sigma}.$$

**Доказательство.** $\mathrm{clip}(\hat\rho, \pm C)$ имеет L2-sensitivity $\Delta = C$ (изменение одного клиента может изменить ρ на максимум $2C$, fasade-sensitivity = $C$ для each-client privacy). Применяем Gaussian mechanism с $\sigma \ge \Delta \sqrt{2\ln(1.25/\delta)}/\varepsilon$, что даёт $(\varepsilon, \delta)$-DP. Перевернув:
$$\varepsilon = \Delta \sqrt{2\ln(1.25/\delta)} / \sigma = C\sqrt{2\ln(1.25/\delta)}/\sigma. \quad \square$$

**Privacy guarantee level (С=50, δ=10⁻³):** $\sqrt{2 \ln(1250)} \approx 3.776$, поэтому
$$\varepsilon_1 \approx 188.8 / \sigma.$$

### Theorem 4c — Composition over T rounds

**Стандартный challenge DP training.** Каждый MeZO-step = одно применение Gaussian mechanism с per-step ε₁. Полная privacy за $T$ rounds **не равна** ε₁, а компонуется.

**Three composition strategies:**

**(i) Basic composition (Dwork-Roth Theorem 3.16).** $T$-кратное применение $(\varepsilon_1, \delta)$-DP механизмов даёт $(T\varepsilon_1, T\delta)$-DP:
$$\varepsilon_T^{\text{basic}} = T \varepsilon_1.$$
Linear в $T$ — для $T = 200$, $\varepsilon_1 = 10$ → $\varepsilon_T = 2000$. Бесполезно.

**(ii) Advanced composition (Dwork-Rothblum-Vadhan 2010).** Для $T$ механизмов $(\varepsilon_1, \delta)$-DP, при любом $\delta' > 0$ имеем $(\varepsilon', T\delta + \delta')$-DP с
$$\varepsilon' = \sqrt{2T \ln(1/\delta')} \cdot \varepsilon_1 + T \varepsilon_1 \bigl(e^{\varepsilon_1} - 1\bigr).$$
$\sqrt{T}$ scaling — для $T=200$, $\varepsilon_1 = 10$, $\delta' = 10^{-3}$:
$$\varepsilon_T^{\text{advanced}} \approx \sqrt{400 \cdot 6.9} \cdot 10 + 200 \cdot 10 \cdot 22025 \approx 525 + 10^7. \quad \text{(catastrophic)}$$
...
### Theorem 4 — Practical interpretation

**Per-round vs per-training privacy:**

- **Per-round (Theorem 4b):** "After one round of federated MeZO, an adversary learns at most ε₁ about any single client's data." This is the standard claim in federated DP setups.
- **Per-training (Theorem 4c):** "After T = 200 rounds, the cumulative privacy loss for any client is ε_T." With current composition tools, this requires σ ~$50 \times \sqrt{T}$ to get ε_T ≤ 10, which makes utility loss explode.

**Practical paper claim:** Report **per-round ε** (the standard convention for one-shot federated fine-tuning), acknowledge composition challenge in Limitations.

### Honest limitations

1. **Malladi r(H)-trick breaks for DP-noise** (Lemma 8 discussion). Theoretical noise floor scales linearly with full $d$, not $r(H)$. Empirically the bound is loose — likely because optimisation does not reach the steady-state floor in $T = 200$ rounds for $\sigma$ tested.

2. **Composition for many rounds is fundamentally hard** without subsampling or RDP+amplification. Federated MeZO без subsampling = full-batch update per round → composition is unavoidable.

3. **Sensitivity Δ = C assumes each client contributes one bounded ρ.** If multi-step locally (`local_steps > 1`), sensitivity grows accordingly. Current implementation uses `local_steps = 1` → safe.

4. **No subsampling amplification used.** Future work: add stochastic mini-batches per round → reduces ε per step by $q$ (batch fraction).

### Connections

- T3 проиграет to vanilla SGD with $\sigma = 0$ (no DP added).
- For σ small (e.g., 0.5): perturbation negligible — should approximately match D-MeZO-N v1.
- For σ large (e.g., 19+): theoretical noise floor explodes; empirical confirmation in §6.7.
...
**T4 (DP-MeZO-N privacy + utility):**
- Dwork & Roth 2014. *The Algorithmic Foundations of Differential Privacy*. Foundations and Trends in Theoretical Computer Science. **Theorem 3.16 (basic composition), Theorem 3.20 (advanced composition), Appendix A.1 (Gaussian mechanism).**
- Mironov 2017 (CSF). *Rényi Differential Privacy*. — **RDP framework**, tighter composition for Gaussian.
- Abadi et al. 2016 (CCS). *Deep Learning with Differential Privacy*. **Moments accountant + subsampling amplification.**
- Tang et al. 2024. *Private Fine-tuning of Large Language Models with Zeroth-order Optimization*. **DP-MeZO predecessor** (centralized).
- Truex et al. 2019 (CCS Workshop). *A hybrid approach to privacy-preserving federated learning*. Federated DP context.
