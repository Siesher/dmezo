...
**Эскиз доказательства.** Из расширенной (A1)-гладкости в обеих компонентах $V_t$ получаем descent-equation $V_{t+1} \leq V_t - \tfrac{3\eta}{4}\|\nabla L_t\|^2 - \tfrac{3\eta(1-\beta_t^2)}{8}\|v_t\|^2 + \eta G^2$ (cross-terms $\pm\eta\beta_t\langle\nabla L_t, v_t\rangle$ из потенциальной и кинетической компонент взаимно сокращаются — именно для этого в $V_t$ выбран множитель $\eta/2$ при кинетике). PL переводит $-\frac{3\eta}{4}\|\nabla L\|^2 \leq -\frac{3\eta\mu}{2}(L - L^{\star})$, а кинетический член контрактирует со скоростью $\frac{3(1-\beta_t^2)}{4}$. Берём минимум двух скоростей контракции $\alpha_t = \min(3\eta\mu/2, 3(1-\beta_t^2)/4)$; при $\eta \leq (1-\beta_0^2)/(8\ell)$ и $\mu \leq \ell$ имеем $\alpha_t \equiv 3\eta\mu/2$ независимо от $\beta_t$. Геометрическое суммирование $\sum_{k=0}^\infty (1-\alpha)^k \eta G^2 = \eta G^2/\alpha = 2G^2/(3\mu)$ даёт neighbourhood. Полное доказательство (шаги (1)–(8), включая явное алгебраическое сокращение cross-terms) — `docs/theory_rigorous.md` §3. ∎
