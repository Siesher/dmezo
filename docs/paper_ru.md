...
Мы представляем **D-MeZO-N** — Decentralized Federated MeZO с ускорением Нестерова — первый полностью peer-to-peer федеративный zeroth-order оптимизатор для дообучения больших языковых моделей. Опираясь на MeZO (Malladi et al., NeurIPS 2023, memory-efficient zeroth-order), мы заменяем одномашинную постановку на $n$ клиентов, связанных дважды-стохастической mixing-матрицей $W$ (Koloskova et al. 2020). Каждый клиент передаёт только один скаляр (проекцию градиента $\rho$) и одно целое число (seed) за раунд каждому соседу — что устраняет гигабайтные обмены градиентами, типичные для FedAvg. Для стабилизации heavy-ball момента Нестерова при высокой дисперсии ZO-оценок градиента мы вводим $\rho$-clipping вместе с линейным расписанием $\beta$-decay; получаем ускоренный вариант с монотонным убыванием. На самой сложной федеративной ячейке D-MeZO-N достигает final loss 0.1291 (single seed), что соответствует улучшению на **6.0%** относительно ablation-control (seed=42 без момента, 0.1373) и **7.9%** относительно 2-seed baseline ring+Dir($\alpha$=0.5) (0.1402). На модели Qwen3.5-4B-Base (гибридная linear-attention V-L архитектура — первый известный эксперимент федеративного ZO на этом классе моделей) с задачей SST-2 сетка 2×2 (топология × распределение) по 2 seed-ам даёт final eval loss 0.1271–0.1507 во всех ячейках, превосходя централизованный MeZO baseline (0.1762) на 14.5–27.9% за счёт неявного усреднения $z$-направлений. Эмпирику дополняют три формальные теоремы сходимости — **Теорема 1** (выпуклый случай + момент, $\rho$-clipping), **Теорема 2** (невыпуклый случай PL без момента) и **Теорема 3** (PL + heavy-ball + $\beta$-decay + $\rho$-clipping) — предсказания которых количественно подтверждены эмпирикой. Весь код, конфиги, MLflow run ID и 128 unit-тестов опубликованы.
...
- **C2** — D-MeZO устойчив к экстремальной неоднородности распределения: партиционная «стоимость» Dirichlet($\alpha$=0.5) **≤ 11.8%** относительно IID (по 2 seed-ам), что **на порядок** ниже типичных 50–200% для FedAvg.
- **C3** — Разница между топологиями $\leq$ 8% при $n=4$ клиентах; контр-интуитивно, ring(4) **лучше** complete(4) на ZO-режиме на **обоих** распределениях (5.7% выигрыш на IID, 7.0% — на Dir($\alpha$=0.5)) — это говорит о неявной регуляризации за счёт более медленного consensus-микширования.
- **C4** — D-MeZO-N (heavy-ball Нестеров + $\rho$-clipping при $C=50$ + линейный $\beta$-decay $0.9 \to 0$) даёт монотонно сходящийся ускоренный вариант; на самой сложной ячейке (ring+Dir($\alpha$=0.5), seed=42) он достигает final 0.1291 (single seed), улучшение на **6.0%** относительно ablation-control без момента (0.1373) и на **7.9%** относительно 2-seed baseline ring+Dir(0.5) (0.1402 ± 0.0029). Относительно централизованного MeZO baseline (0.1762) улучшение **26.7%**.
- **C5** — Три формальные теоремы сходимости: **Теорема 1** (выпуклый случай + момент, $\rho$-clipping, federated speedup $1/\sqrt{nT}$), **Теорема 2** (невыпуклый PL без момента, линейная сходимость к neighbourhood), **Теорема 3** (PL + heavy-ball + $\beta$-decay + $\rho$-clipping; Lyapunov $V_t = (L-L^{\star}) + (\eta/2)\|v\|^2$, neighbourhood $4G^2/(3\mu)$).
- **C6** — Восемь количественно проверяемых предсказаний из теорем эмпирически подтверждены: federated speedup, blow-up без clip, look-ahead двойное noise channel, late drift R1b, monotonic R1d, линейная PL-сходимость, consensus-штраф $\rho^2/(1-\rho)^2$, ZO bias $O(\epsilon^2)$.

Восемь предсказаний из Теорем 1–3 (линейное федеративное ускорение, расходимость $\beta=0.9$ без clip, late drift у R1b, монотонное убывание у R1d, линейная сходимость к шумовому floor, $1/n$ стохастический floor, consensus-штраф $\rho^2/(1-\rho)^2$, ZO bias $\sim \epsilon^2$) количественно подтверждены эмпирическими прогонами (детали — Таблица 1, §4.6).
...
|---|---|---|---|---|---|
| Centralized vanilla MeZO | 2.8493 → 1.4331 | **−49.7%** | 0.3750 → 0.3750 | 0.3750 | PASS (loss lower) |
| **Federated D-MeZO-N v1** | 2.8493 → **1.5155** | **−46.8%** | 0.3750 → **0.3875** | **0.4125 @R500** | PASS (acc higher) |
...
Один и тот же recipe (β-decay 0.9 → 0 + ρ-clip 50) демонстрирует на single-seed evidence **rescue-like поведение** когда vanilla расходится (HellaSwag: $|\hat\rho|$ пик +159, neighborhood разъезжается) и **Pareto trade-off** когда vanilla сходится (MathLogicQA: vanilla дёт лучшее loss −49.7%, D-MeZO-N даёт лучше accuracy +1.25pp но хуже loss −46.8% — методы дают разные точки на Pareto-frontier, не universal D-MeZO-N win). $|\hat\rho|$ пик на MathLogicQA +375 не вреден потому что кумулятивный эффект ограничен single-token suffix loss. Эти findings — directional evidence, согласованное с Theorem 3 corollary, но не statistically definitive (см. §6.9); multi-seed validation планируется в follow-up работе.
