...
Один и тот же recipe (β-decay 0.9 → 0 + ρ-clip 50) демонстрирует на single-seed evidence **rescue-like поведение** когда vanilla расходится (HellaSwag: $|\hat\rho|$ пик +159, neighborhood разъезжается) и **safe-tracking** когда vanilla сходится (MathLogicQA: $|\hat\rho|$ пик +375, кумулятивный эффект ограничен single-token suffix loss). Эти findings — directional evidence, согласованное с Theorem 3 corollary, но не statistically definitive (см. §6.9); multi-seed validation планируется в follow-up работе.
