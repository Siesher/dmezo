...
Однако существующая литература по федеративному MeZO (FedKSeed, Ferret, FedZeN) ограничена (а) единой full-attention архитектурой (семейство OPT, LLaMA), и (б) центрально-агрегированной топологией FedAvg (McMahan et al. 2017). Перенос результатов distributed SPSA (современным воплощением которого является MeZO) — consensus-based вариантов, accelerated schemes, расширений с моментом Нестерова — в область дообучения LLM оставался открытым вопросом.
...
**SPSA — foundational ancestor.** Simultaneous Perturbation Stochastic Approximation (Spall 1992, IEEE TAC 37:332) — двухточечный zeroth-order оптимизатор, лежащий в основе всей последующей ZO-литературы. Original SPSA использует Bernoulli ±1 perturbation $\Delta$ и обновление $\theta \leftarrow \theta - \eta \cdot ((L(\theta+\varepsilon\Delta) - L(\theta-\varepsilon\Delta))/(2\varepsilon)) \cdot \Delta^{-1}$. Convergence rate $O(1/\sqrt{T})$ (Sadegh-Spall 1998); 2-SPSA с Hessian estimation — Spall 2000; adaptive gain schedule $\eta_t = a/(t+A)^\alpha$ ($\alpha \approx 0.602$) — Spall 1997. SPSA не предназначен для масштабов LLM из-за explicit storage возмущения $\Delta$ ($O(d)$ память) — этот gap закрывает MeZO.

**MeZO — SPSA для LLM.** Malladi et al. (2023, NeurIPS) ввели MeZO как SPSA с двумя ключевыми innovations: (i) **Gaussian $z \sim \mathcal{N}(0, I)$** вместо Bernoulli (даёт $r(H)$-trick, ниже), (ii) **in-place seed-reconstructed $z$** — direction $z$ регенерируется детерминированно из seed-а, не материализуется как тензор → память остаётся inference-level. Они показали дообучение OPT-{1.3B, 13B, 30B, 66B} на SuperGLUE с памятью, сравнимой с инференсом. Теорема 3.1 доказывает variance bound через эффективный ранг гессиана $r(H) := \mathrm{tr}(H)/\|H\|_{op}$ вместо полной размерности $d$ — это и обеспечивает применимость на LLM-масштабе ($r(H) \ll d$ для overparametrized трансформеров).

**$\ell_2$-smoothing анализ (российская школа).** Параллельно с SPSA развивалась традиция Gaussian/uniform smoothing для convex ZO (Nesterov, Spokoiny 2017, FOCM). Gasnikov et al. (2023, "Randomized gradient-free methods in convex optimization", *Encyclopedia of Optimization*, arXiv:2211.13566) систематизировали этот фреймворк: two-point estimator $\nabla f_\gamma(x, e) = d \cdot \frac{f(x+\gamma e) - f(x-\gamma e)}{2\gamma} \cdot e$ для $e \sim \mathrm{Unif}(S_2^d)$, bias bound $f(x) \le f_\gamma(x) \le f(x) + \gamma M_2$ и variance bound $\mathbb{E}\|\nabla f_\gamma\|^2 \le \sqrt{2}\min(q, \ln d) d^{2/q} M_2^2$. Эти оценки — формальный фундамент наших convergence proofs T1-T4. Survey покрывает convex/strongly convex случаи с accelerated rates; PL + heavy-ball режим (на котором стоит D-MeZO-N) survey не охватывает — это наш delta.

**D-MeZO-N — SPSA + MeZO + 4 новых элементов.** Наш метод строится на SPSA-родословной (34 года теории stochastic approximation) и добавляет: (1) federated wrapper с independent per-client $z_i$ + gossip mixing matrix $W$ (vs FedKSeed-style shared seed pool), (2) heavy-ball scalar momentum с adaptive ρ-clip + drift-reset (vs SPSA's adaptive gain без momentum), (3) closed-form Lyapunov convergence proof под PL — первая гарантия устойчивости ZO heavy-ball в PL-режиме (Theorem 3; скорость сжатия совпадает с plain SGD под PL, асимптотическое ускорение не заявляется), (4) dual-use ρ-clip как L2-sensitivity для Gaussian-mechanism DP (targeted search для "DP-SPSA" вернул empty — open niche).

**Децентрализованный SGD.** Koloskova et al. (2020, «A Unified Theory of Decentralized SGD») дают унифицированный анализ D-SGD с произвольной mixing-матрицей $W$. Их Теорема 2 (выпуклый случай) и Теорема 8 (PL) ограничивают rate сходимости в зависимости от спектральной щели $\rho(W)$ и неоднородности градиентов $\zeta^2$; фреймворк включает методы с локальными обновлениями (Stich 2019). Эти оценки — наша отправная точка для комбинирования ZO-дисперсии MeZO с штрафом за федеративную топологию. Параллельно, Beznosikov, Richtárik et al. (NeurIPS 2022, arXiv:2207.10792) развивают decentralized SGD с communication compression для вариационных неравенств — VI-аналог нашей «16 байт/раунд» экономии; Sadiev et al. (2022, *EURO J Comp Opt*) дают lower bounds для decentralized personalized FL, ограничивающие optimality нашего T1 снизу.

**Федеративный zeroth-order.** FedKSeed (Qin et al., ICML 2024) и Ferret (Shu et al., 2024) — оба строятся на MeZO для FL, используя общие словари seed-ов для дальнейшего сжатия коммуникации. FedZeN (Maritan et al. 2024) исследует Newton-стиль zeroth-order в FL. Все три работы ограничены (i) full-attention архитектурами и (ii) центрально-агрегированной FedAvg топологией; ни одна из них не рассматривает peer-to-peer децентрализованный случай с heavy-ball Nesterov-стабилизацией. DeComFL (Li et al. 2024, ICLR 2025) аналогично сжимает FL-коммуникацию до $O(1)$ скаляров за раунд через shared-seed zeroth-order и доказывает dimension-free rate при low-effective-rank, но остаётся server-orchestrated с одним общим seed на раунд. D-MeZO-N отличается по всем трём осям: peer-to-peer gossip по любой дважды-стохастической $W$, **независимые** $z_i$ у каждого клиента (источник $1/n$ редукции по обеим компонентам шума, §4.4) и анализ устойчивости момента (Теорема 3).
...
где $z \sim \mathcal{N}(0, I)$ восстанавливается из seed-а $s$. Полный раунд D-MeZO-N комбинирует $\rho$-clip шаг, heavy-ball (Polyak 1964) обновление скорости с (возможно расписанным) коэффициентом момента $\beta_t$, шаг по параметрам и consensus mixing:
...
За раунд каждому соседу клиент передаёт текущее $\hat\rho_i$ (один float) и seed $s_i$ (одно целое число) — 16 байт. Для модели на 4 миллиарда параметров в bf16 (8 ГБ плотного градиента) это $8\,\text{ГБ}/16\,\text{байт} = 5\cdot10^8$-кратное сжатие (почти девять порядков) по сравнению с обменом плотными градиентами в FedAvg:
...
- **(A2 / PL, для Теорем 2 и 3):** $\|\nabla L(\theta)\|^2 \geq 2\mu \bigl(L(\theta) - L^{\star}\bigr) \quad \forall \theta \in \mathbb{R}^d.$

## 4.2 Набор лемм

**Лемма 1** (Nesterov-Spokoiny / Malladi ZO-variance; H-weighted form). *В условиях (A1)+(C5) двухточечная ZO-оценка $\hat g = \hat\rho\, z$ с $z \sim \mathcal{N}(0,I)$ для любой PSD-матрицы $M$ удовлетворяет H-взвешенной квадратичной оценке (theory_rigorous.md Лемма 1b′):*

$$\mathbb{E}_z \bigl[\hat g^\top M \hat g\bigr] \leq \|M\|_{op}\,\|\nabla L\|^2\,(r(M) + 2) + O(\epsilon^2),$$

*в частности для $M = H$: $\mathbb{E}_z[\hat g^\top H \hat g] \leq \ell\,(r(H) + 2)\,\|\nabla L\|^2 + O(\epsilon^2)$. Смещение $\| \mathbb{E}[\hat g] - \nabla L(\theta) \| \leq \tfrac{\ell_3}{2}\epsilon^2(d+2)$ (Лемма 1a). Замена $d$ на $r(H)$ — ключевая идея Malladi (2023), делающая ZO применимым на масштабе LLM (effective rank $r(H) \ll d$ для overparameterised трансформеров). **Важное уточнение:** сырой евклидов второй момент масштабируется с **полной** размерностью — $\mathbb{E}_z\|\hat g\|^2 \leq (d+2)\|\nabla L\|^2 + O(\epsilon^2)$ (Лемма 1b); послабление effective-rank $r(H)$ применимо только внутри $H$-взвешенных квадратичных форм, и именно там его использует descent-анализ.*
...
**Лемма 3** (consensus error в стиле Колосковой; = theory_rigorous.md Лемма 4). *Для D-MeZO-N с mixing-матрицей $W$ ($\rho_W < 1$) и моментом $\beta_t$ (с $\bar\beta := \beta_0/2$):*

$$\frac{1}{n} \sum_i \| \theta_i^{t+1} - \bar\theta_{t+1} \|^2 \leq \frac{\rho_W^2}{(1-\rho_W)^2} \cdot \frac{\eta^2 \bigl( G^2 r(H) + \zeta^2 \bigr)}{(1-\bar\beta)^2}.$$
...
*Три слагаемых: стохастическое linear-speedup в $n$ клиентов ($1/\sqrt{nT}$), consensus penalty с mixing-амплифайером $1/(1-\rho_W)^2$ и momentum-амплифайером $1/(1-\bar\beta)^2$ (зануляется для complete graph где $\rho_W = 0$), ZO-bias старшего порядка по $\epsilon$. Здесь $D_0 = \|\bar\theta_0 - \theta^{\star}\|^2$; $\tilde O$ скрывает $\ell$ и логарифмические факторы.*

**Эскиз доказательства.** Полное доказательство — `docs/theory_rigorous.md` §4. В выпуклом случае константы PL $\mu$ не существует, поэтому **геометрическая контракция через Ляпунов недопустима**; используется стандартный distance-to-optimum route для выпуклого SGD с усреднением Поляка-Рупперта $\hat\theta_T := \frac{1}{T}\sum_t \bar\theta_t$.

*Шаг 1 (раскрытие расстояния до оптимума).* По обновлению consensus-усреднённой итерации:
$$\|\bar\theta_{t+1} - \theta^{\star}\|^2 = \|\bar\theta_t - \theta^{\star}\|^2 - 2\eta\langle \bar v_{t+1}, \bar\theta_t - \theta^{\star}\rangle + \eta^2\|\bar v_{t+1}\|^2.$$

*Шаг 2 (выпуклость + consensus drift).* Берём $\mathbb{E}[\cdot|\mathcal{F}_t]$, выделяем $\mathbb{E}\bar v_{t+1} = \nabla L(\bar\theta_t) + \xi_t + b_t$ с consensus drift $\xi_t = \frac{1}{n}\sum_i(\nabla L_i(\theta_i^t) - \nabla L_i(\bar\theta_t))$, $\|\xi_t\| \leq \ell\sqrt{\Pi_t}$ (ограничен Леммой 3). По **выпуклости** $\langle \nabla L(\bar\theta_t), \bar\theta_t - \theta^{\star}\rangle \geq L(\bar\theta_t) - L^{\star}$ — это заменяет PL-неравенство (никакой $\mu_{\text{eff}} \propto 1/\sqrt{T}$ контракции не вводится).

*Шаг 3 (telescoping + Jensen).* Суммируем по $t = 0, \dots, T-1$, делим на $2\eta T$, применяем неравенство Йенсена для $\hat\theta_T$:
$$\mathbb{E}[L(\hat\theta_T) - L^{\star}] \leq \frac{D_0}{2\eta T} + \underbrace{\text{drift term (I)}}_{\text{Лемма 3}} + \underbrace{\text{variance term (II)}}_{\text{per-client } 1/n},$$
где $D_0 = \|\bar\theta_0 - \theta^{\star}\|^2$, (II) $\leq \frac{\eta C^2 r(H)\ell}{2n(1-\bar\beta)^2}$ (federated speedup $1/n$), (I) $\leq \frac{\rho_W^2 C^2 r(H)\ell}{(1-\rho_W)^2(1-\bar\beta)^2 T}\cdot\text{const}$ (consensus penalty).

*Шаг 4 (оптимизация $\eta$).* Минимизация $\frac{D_0}{2\eta T} + \frac{\eta C^2 r(H)\ell}{2n}$ по $\eta^{\star} = \sqrt{D_0 n/(C^2 r(H)\ell T)}$ даёт rate $\tilde O\!\bigl(\sqrt{D_0 C^2 r(H)\ell/(nT)}\bigr)$ плюс consensus penalty $\tilde O(\rho_W^2 C^2 r(H)\ell/((1-\rho_W)^2 T))$ плюс ZO-bias $O(\epsilon^2 \ell^2 r(H))$. ∎
...
*где $V_t := (L(\theta_t) - L^{\star}) + (\eta/2)\|v_t\|^2$ — Ляпуновская функция комбинирующая потенциал и кинетику. Поскольку $L(\theta_T) - L^{\star} \leq V_T$, тот же bound выполняется и для loss-компоненты:*

$$
\mathbb{E}[L(\theta_T) - L^{\star}] \;\leq\; (1 - \tfrac{3\eta\mu}{2})^T V_0 \;+\; \tfrac{2 G^2}{3\mu}.
$$

**Эскиз доказательства.** Из расширенной (A1)-гладкости в обеих компонентах $V_t$ получаем descent-equation $V_{t+1} \leq V_t - \tfrac{3\eta}{4}\|\nabla L_t\|^2 - \tfrac{3\eta(1-\beta_t^2)}{8}\|v_t\|^2 + \eta G^2$ (cross-terms $\pm\eta\beta_t\langle\nabla L_t, v_t\rangle$ из потенциальной и кинетической компонент взаимно сокращаются — именно для этого в $V_t$ выбран множитель $\eta/2$ при кинетике). PL переводит $-\frac{3\eta}{4}\|\nabla L\|^2 \leq -\frac{3\eta\mu}{2}(L - L^{\star})$, а кинетический член контрактирует со скоростью $\frac{3(1-\beta_t^2)}{4}$. Берём минимум двух скоростей контракции $\alpha_t = \min(3\eta\mu/2, 3(1-\beta_t^2)/4)$; при $\eta \leq (1-\beta_0^2)/(8\ell)$ и $\mu \leq 4\ell$ имеем $\alpha_t \equiv 3\eta\mu/2$ независимо от $\beta_t$. Геометрическое суммирование $\sum_{k=0}^\infty (1-\alpha)^k \eta G^2 = \eta G^2/\alpha = 2G^2/(3\mu)$ даёт neighbourhood. Полное доказательство (шаги (1)–(8), включая явное алгебраическое сокращение cross-terms) — `docs/theory_rigorous.md` §3. ∎
...
| P6 | Линейная сходимость $(1-\eta\mu/2)^T$ (T2) | Геом. спад к noise floor | Ring+IID: 3.56 → 0.126 | ✓ |
...
Эмпирическое соотношение $0.1271 / 0.1762 = 0.722$ согласуется по направлению с федеративным variance reduction — федеративные клиенты с независимыми $z_i$ выигрывают относительно централизованного MeZO. Мы **не утверждаем** количественное совпадение с предсказанием $1/\sqrt{n} = 0.5$ из Теоремы 1: разница $0.722 \ne 0.5$ остаётся необъяснённой (вероятно, зависит от $r(H)$-зависимого константного множителя), и прямого переноса теоретического rate на финальный loss-ratio нет. Механизм: когда $n$ клиентов независимо делают MeZO-замер своим собственным seed-ом $s_i$ и направлением $z_{s_i}$, consensus-усреднение даёт несмещённое среднее $n$ независимых unit-direction замеров. Стандартный анализ variance reduction (дисперсия $\div n$) показывает, что weight_avg consensus фактически делает параллельное multi-direction MeZO при том же бюджете forward-passes, что и централизованный single-direction MeZO. Это «бесплатная выгода» для федеративного обучения в ZO-режиме — обратная обычной FL-формулировке, где федеративный setup воспринимается как cost-paying. Количественное совпадение с теоретическим $1/\sqrt{n}$-rate оставляем для будущей работы.
...
> **Соглашение о std.** Везде в статье «±» обозначает популяционное стандартное отклонение (ddof = 0, знаменатель $n = 3$ по seed-ам); выборочное std (ddof = 1) примерно на 22% больше и не меняет ни одного утверждения о направлении эффекта.

| Variant | s=42 loss/acc | s=43 loss/acc | s=44 loss/acc | Mean ± std loss | Mean ± std acc | Δ vs vanilla | Direction (3 seeds) |
|---|---|---|---|---|---|---|---|
| **vanilla MeZO** | 1.3747 / 0.38 | 1.3432 / 0.36 | 1.3863 / 0.39 | **1.3681 ± 0.018** | **0.377 ± 0.012** | reference | — |
| D-MeZO-N v1 (fixed $C=50$) | 1.4598 / 0.38 | 1.4569 / 0.36 | 1.4735 / 0.39 | 1.4634 ± 0.007 | 0.377 ± 0.012 | +7.0% loss / 0pp acc | **3/3 worse** (falsified) |
| Drift-only (B5, 53 resets) | 1.4608 / 0.38 | 1.4531 / 0.36 | 1.4537 / 0.39 | 1.4559 ± 0.003 | 0.377 ± 0.012 | +6.4% loss / 0pp acc | **3/3 worse** (falsified) |
...
**Главный результат**: D-MeZO-N v2 = combo (B1 adaptive_clip + B5 drift-reset) **устойчиво превосходит vanilla MeZO** на 3-seed paired validation:
- **Loss**: 3/3 same direction (Δ ∈ {−7.0%, −3.6%, −6.0%}, mean = −5.5%)
- **Accuracy**: mean +2.3pp (per-seed Δ ∈ {−1pp, +8pp, 0pp} — никогда существенно не теряет)
- **Наименьший std loss среди вариантов, улучшающих vanilla** (0.010 vs B1 alone 0.021; сам vanilla 0.018) — additional robustness metric. (Drift-only имеет ещё меньший std loss 0.0035, но это артефакт его равномерного «застревания» на худшем loss +6.4%, а не стабильность хорошего решения.)
...
| **MathLogicQA (Qwen3.5-4B-Base)** | **сходится (1.368 ± 0.018)** | **выигрывает 1.293 ± 0.010** | **безопасное отслеживание + выигрыш** | **n=3 paired, ROBUST** ⭐ |
...
- **MLflow run IDs** (Drive-mirrored) для каждой числовой величины в Таблицах 1–2 и на рисунках раздела §5.
...
- **§5.6 D-MeZO-N v1 single-seed safe-tracking на MathLogicQA/Qwen3.5-4B-Base** — устаревший single-seed результат (+1.25 pp acc), **superseded** 3-seed paired валидацией §5.6.2 (Robust, см. §8 A7): combo v2 robustly beats vanilla по loss (−5.5%, 3/3), accuracy +2.3pp статистически незначима. Этот single-seed пункт более не актуален.
...
- **Richardson 4-point / 6-point** (§6.7 supplement) — sweet-spot и ranking observation, не fully validated cross-task. (Архивные JSON хранят только losses; verifiable по архиву эффект — это loss: richardson_eq 0.854 vs 2-pt 0.929 на Qwen3-0.6B, т.е. −8.1%. Любые accuracy-числа Richardson взяты из run-логов и по архивному JSON не проверяемы.)
...
Local ablation на Qwen3.5-0.8B / MathLogicQA / 2 seeds выявил **accuracy paradox B1 alone** (loss-parity vs vanilla, но −17pp acc) и установил combo как кандидат. (При $n=2$ seeds paired bootstrap имеет лишь 3 различных resample, поэтому соответствующий интервал на $\Delta$acc — консервативный диапазон, а не калиброванный CI.) **Paper-scale 3-seed paired validation** на Qwen3.5-4B-Base / MathLogicQA подтвердил:
...
| B5 alone | 1.456 ± 0.003 | 0.377 | +6.4% worse | 3/3 worse |
...
**Наименьший std loss среди вариантов, улучшающих vanilla** (0.010 vs B1 alone 0.021; сам vanilla 0.018) — combo достигает **stability** через mechanism complementarity. (Drift-only имеет ещё меньший std loss 0.0035, но он равномерно «застрял» на худшем loss +6.4% — это не стабильность хорошего решения, а артефакт.)
...
> **Доступность данных.** Сырые JSON σ-sweep лежат в архиве Colab-запуска (`dmezo_runs/` на Drive, run от 2026-05-20); sweep воспроизводим через `scripts/sweep_dp_sigma.py`. В репозитории `experiments/diagnostics/` покрывает одну DP-конфигурацию ($\sigma = 0.5$).
...
**Главное наблюдение:** **frontier статистически плоский** во всём диапазоне $\sigma \in [0.5, 50]$. Все CI пересекаются (при $n=2$ seeds paired bootstrap имеет лишь 3 различных resample; при $n=3$ — 10, поэтому приводимые интервалы — консервативные диапазоны, а не калиброванные CI). Acc при ε=10 (0.265) **идентичен** no-DP baseline (0.265).

**Headline claim:** "Первый decentralized federated ZO оптимизатор с формальной $(\varepsilon=10, \delta=10^{-3})$-DP гарантией **за раунд** на LLM с $\sim 6\%$ utility cost." **Важное ограничение: это per-round гарантия; T-round composition через базовую теорему даёт $\varepsilon_T = T \cdot \varepsilon_1 \approx 2000$ при $T=200$ (бесполезно для end-to-end DP). RDP/Moments accountant даёт $O(\sqrt{T})$ — всё ещё большое при long training. Subsampling amplification (Abadi 2016) — recommended future direction для практического end-to-end DP.**

**Почему теоретический шум-пол T4 не наблюдается?** Theorem 4 (см. `docs/theory_rigorous.md` §6.5, Lemma 8) предсказывает шум-пол $\frac{2(C^2 r(H) + \sigma^2 d)\,\ell}{3\mu}$ — clip-noise выровнен с градиентом и масштабируется с effective rank $r(H)$, а DP-noise $\xi z$ изотропен и масштабируется с **полной** размерностью $d$ (в коде scalar noise добавляется к $\hat\rho$ после клипа, `src/dmezo/mezo/step.py:193`). При $\sigma = 0$ эта оценка точно сводится к T3. Член $\sigma^2 d$ должен **доминировать** над $C^2 r(H)$ для $\sigma > C\sqrt{r(H)/d} \approx 0.02$. Однако:
...
- Advanced (Dwork-Rothblum-Vadhan 2010): формально даёт $\sqrt{T}$-scaling, но второй член $T \varepsilon_1 (e^{\varepsilon_1}-1)$ взрывается при $\varepsilon_1 = 10$ (фактически $\sim 10^7$). Advanced/RDP composition даёт реальную экономию **только в режиме $\varepsilon_1 \ll 1$** (где $e^{\varepsilon_1} - 1 \approx \varepsilon_1$); при $\varepsilon_1 = 10$ доминирует **basic** composition.
- **RDP / Moments accountant** (Mironov 2017, Abadi 2016): tighter via Rényi. Per-round RDP-коэффициент Gaussian-механизма **включает L2-чувствительность**: $\varepsilon_\alpha = \alpha\,C^2/(2\sigma^2)$ (а не $\alpha/(2\sigma^2)$); после $T$ раундов $(\alpha, T\alpha C^2/(2\sigma^2))$-RDP, что всё ещё $O(\sqrt{T})$ для Gaussian.
- **Subsampling amplification** (Abadi 2016): уменьшает per-step ε на batch fraction $q$ — straightforward extension для future work.

**Что нужно для useful training-level budget:** достижение полезного бюджета на всё обучение требует per-round $\varepsilon_1 < 1$ (т.е. бо́льшего $\sigma$) — именно этот режим должна целить полная трактовка через RDP-accountant (future work).
...
| **B2.** D-MeZO-N v2 transient 3× speedup на SST-2 (Day 8 R1b R100→R300) | n=1 seed | Не доказан формально (открытый вопрос: transient acceleration в ZO heavy-ball; asymptotic acceleration запрещено Bottou-Curtis-Nocedal 2018) |
...
| **D6.** Full decentralized Theorem 3 (открытый вопрос: decentralized extension of T3) | Theory | 60–80h |
...
**Доступность кода и данных.** Весь код, конфиги, тесты и закоммиченные сырые JSON результатов доступны в репозитории `github.com/Siesher/dmezo`. Пути вида `docs/…`, `scripts/…`, `tests/…`, `notebooks/…`, упоминаемые в статье, указывают на файлы в этом репозитории; в частности, полные доказательства — в `docs/theory_rigorous.md`, headline-данные — в `experiments/diagnostics/validate_multiseed_fed_Qwen_Qwen3p5-4B-Base_mathlogicqa.json` и `head-to-head/head_to_head_fedkseed_Qwen_Qwen3p5-4B-Base_mathlogicqa.json`.
